...
Let $\mathcal{F}_n$ denote a $\sigma$-algebra that contains $X$, and we index each with $n$. Then the $\sigma$-algebra generated by $X$ is defined as $\sigma(X) = \cap_{i\in \mathbb{N}} \mathcal{F}_i$.
...
**Exercise**: Show that $\mathcal{B}$ as generated by $B$, can also be generated by the following set: $B_\mathbb{Q} := \\{(a,b): -\infty \leq a < b \leq \infty\\, \forall a,b\in\mathbb{Q}\\}$.

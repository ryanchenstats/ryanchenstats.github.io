...
Now define $\|\cv{T}_n\|$ as the maximum interval length of the partition. As $n\to\infty$ then $\| \cv{T}_n \| \to 0$. Furthermore, for any interval $[t_j,t_{j+1}) \in \cv{T}_n$, we have $|t_{j+1} - t_j| \leq \|\cv{T}_n\|$. 
...
$$
Var\left(\sum_{(t_j ,t_{j+1}) \in \cv{T}_n} (W_{t_{j+1}} - W_{t_j})^2\right) \leq \sum_{(t_j ,t_{j+1}) \in \cv{T}_n} 2 \|\cv{T}_n\|(t_{j+1}-t_j)
$$
...
Note the quadratic variation of $t$ on any interval is 0, since $\sum_{(t_j ,t_{j+1}) \in \cv{T}_n} (t_{j+1}- t_j)^2 \leq \|cv{T}_n\|\sum_{(t_j ,t_{j+1}) \in \cv{T}_n} (t_{j+1}- t_j) \to 0$ as $n\to\infty$. By the same rational, it is straightfoward to show $[W_t, t](T) = 0$ too on a bounded set. Quadratic variation has to do with realized volatility when tracking stock prices.

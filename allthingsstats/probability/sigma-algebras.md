...
To show a function is $\mathcal{A}/\mathcal{B}$-measurable, and $\mathcal{B}$ is generated by $B$, then it suffices to show that the preimage of all $b \in B$ (the generating set) is an element of $\mathcal{A}$. This is another critical result showing that we only need to consider the generating set to establish measurability. Formally stated, if $T: \mathcal{A} \to \mathcal{B}$ where $\mathcal{B} := \sigma(\mathcal{F})$ is generated by $\mathcal{F}$, then if $T^{-1}(F) \in \mathcal{A}$ for all $F \in \mathcal{F}$, $T$ is $\mathcal{A}/\mathcal{F}$-measurable.
